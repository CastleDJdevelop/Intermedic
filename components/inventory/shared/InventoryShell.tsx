"use client";

import { useState } from "react";
import "@/components/site/site.css";
import { InventoryContextProvider } from "./InventoryContext";
import { Sidebar } from "./Sidebar";
import { Topbar } from "./Topbar";

/**
 * Layout compartido de todo el módulo de Inventario (sidebar fija + topbar).
 * Reutiliza el mismo sistema de estilos de Sitio/CRM (.im-*, site.css) en vez
 * de crear una tercera hoja de estilos paralela.
 */
export function InventoryShell({ children }: { children: React.ReactNode }) {
  const [dark, setDark] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <InventoryContextProvider>
      <div className={`im-root${dark ? " im-dark" : ""}`} style={{ display: "flex" }}>
        <Sidebar dark={dark} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
        <div className="inv-main" style={{ flex: 1, minWidth: 0, marginLeft: 236 }}>
          <Topbar dark={dark} setDark={setDark} setMobileOpen={setMobileOpen} />
          <div style={{ padding: "24px 26px 60px" }}>{children}</div>
        </div>
        <style>{`@media (max-width: 900px) { .inv-main { margin-left: 0 !important; } }`}</style>
      </div>
    </InventoryContextProvider>
  );
}
